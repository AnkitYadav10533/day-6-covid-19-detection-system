...
with st.sidebar:
    ...
    if model_exists:
        st.success("✅ Model weights loaded (`model.h5` found)")
        if st.button("Delete saved model weights"):
            os.remove(MODEL_PATH)
            st.session_state.use_random_model = False
            st.rerun()
    else:
        st.warning("⚠️ `model.h5` not found in workspace.")
        st.markdown(
            "This model can be generated by training using the original python script: "
            "`project_15_run_on_colab_detection_of_covid_19_from_chest_x_ray.py`"
        )
        
        # Uploading model.h5 through UI
        uploaded_model_file = st.file_uploader("Upload model.h5 file directly", type=["h5"])
        if uploaded_model_file is not None:
            with st.spinner("Saving uploaded model..."):
                with open(MODEL_PATH, "wb") as f:
                    f.write(uploaded_model_file.getbuffer())
            st.success("🎉 Saved model successfully!")
            st.rerun()
...
def load_keras_model(path, use_random=False):
    if use_random:
        ...
        try:
            ...
            return model, "Trained Model (model.h5)"
...
if os.path.exists(MODEL_PATH) and not st.session_state.use_random_model:
    with st.spinner("Loading trained model (this may take a few seconds)..."):
        model, model_name = load_keras_model(MODEL_PATH, use_random=False)
elif st.session_state.use_random_model:
    with st.spinner("Initializing neural network..."):
        model, model_name = load_keras_model(MODEL_PATH, use_random=True)
...
if model is None:
    ...
    <div class="glass-card">
        ...
        <p>To run predictions, please do one of the following:</p>
        <ul>
            <li>Run the training python script to generate <code>model.h5</code> in this directory.</li>
            <li>Upload a pre-trained <code>model.h5</code> file in the sidebar.</li>
            <li>Click <b>"Initialize Untrained Model"</b> in the sidebar to test the application interface.</li>
        </ul>
    </div>
    """, unsafe_allow_html=True)
else:
